# mysql数据库安装
&emsp;&emsp;数据库的安装有多种，这里以 `yum` 为例，安装 `mysql 5.7`，默认yum只能安装mysql 5.1。

&emsp;&emsp;要对 189、190、166三个机器进行安装
...
&emsp;&emsp;注意，如果要使用5.7 或者其他任何版本，只能有一个是 `enabled=1`的，其他的都得`enabled=0`。
```shell
...
```
&emsp;&emsp; ok，说明已经有了，下面就可以安装mysql5.6了
4. 写入mysql需要验证`GnuPG`签名
```shell
